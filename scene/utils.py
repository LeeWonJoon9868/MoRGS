...
from RAFT.config.parser import json_to_args
from RAFT.core.utils.flow_viz import flow_to_image
from RAFT.core.utils.utils import load_ckpt

from RAFT.custom import demo_custom

from sam2.build_sam import build_sam2_video_predictor

def get_sam2_model(sam2_checkpoint, model_cfg):
...
def swap_elements(lists, i, j):
    for lst in lists:
        lst[i], lst[j] = lst[j], lst[i]

def get_sam2_mask(predictor, gt_images, optical_flow_masks, ann_frame_idx = 0, ann_obj_id = 1, prev_inference_state=None):
    
    #swap_elements([gt_images, optical_flow_masks], 0, 1)
    ...
    with torch.no_grad():
      ...
      #inference_state = predictor.init_state3(images=gt_images,prev_inference_state=prev_inference_state)
      
      inference_state = predictor.init_state3(images=gt_images)
      predictor.reset_state(inference_state)
      
      for idx, mask in enumerate(optical_flow_masks):
      ...
      for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state, start_frame_idx=0):
          ...
          # }
          masks_bin = (out_mask_logits > 0.0)
          merged_mask = masks_bin.any(dim=0).squeeze(0)
          video_segments.append(merged_mask)
          
      # optical_flow_mask = optical_flow_masks[0] # optical_flow_masks #optical_flow_masks[0]


      # fg = torch.nonzero(optical_flow_mask, as_tuple=False)   # (N_fg, 2), order=(y,x)
      # bg = torch.nonzero(~optical_flow_mask, as_tuple=False)  # (N_bg, 2)

      # if fg.shape[0] > 0:
      #     fg_idx = torch.randint(low=0, high=fg.shape[0], size=(min(n, fg.shape[0]),), device=device)
      #     fg_pts = fg[fg_idx]
      # else:
      #     fg_pts = torch.zeros((0, 2), device=device, dtype=torch.long)

      # if bg.shape[0] > 0:
      #     bg_idx = torch.randint(low=0, high=bg.shape[0], size=(min(n, bg.shape[0]),), device=device)
      #     bg_pts = bg[bg_idx]
      # else:
      #     bg_pts = torch.zeros((0, 2), device=device, dtype=torch.long)
      
      # fg_pts = fg_pts[:, [1, 0]].float()
      # bg_pts = bg_pts[:, [1, 0]].float()


      # points = torch.cat([fg_pts, bg_pts], dim=0)   # (N, 2)
      # #points = fg_pts
      # labels = torch.cat([
      #     #torch.ones(len(fg_pts), dtype=torch.int32, device=device)
      #     torch.ones(len(fg_pts), dtype=torch.int32, device=device),
      #     torch.zeros(len(bg_pts), dtype=torch.int32, device=device)
      # ], dim=0)
      ...
      #     obj_id=ann_obj_id,
      #     points=points[None],   # (1, N, 2) batch 차원 추가
      ...
      # video_segments = []  # 각 프레임별 binary mask (numpy)

      # for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state,start_frame_idx=0):
      #   # out_mask_logits: (num_objects, 1, H, W) 또는 (num_objects, H, W)
      #   # → binary mask로 변환 후 OR 연산으로 합치기
      #   masks_bin = (out_mask_logits > 0.0)  # bool tensor
      #   merged_mask = masks_bin.any(dim=0).squeeze(0)  # (H, W)
    ...
    #swap_elements(video_segments, 0, 1)
...
def calc_flow(args, model, image1, image2):
    ...
    if flow_down.shape[2:] != image1.shape[2:]:
      H0, W0 = image1.shape[2], image1.shape[3]
      flow_down = F.interpolate(flow, size=(H0, W0), mode='bilinear', align_corners=False)
      info_down = F.interpolate(info, size=(H0, W0), mode='area')# v (y)
    return flow_down, info_down


def get_flow_model(path, cfg_file):
    # model = RAFT.from_pretrained(args.url, args=args)
    # parser = argparse.ArgumentParser()
    # parser.add_argument('--cfg', type=str, required=True, default=)
    # args = parse_args(parser)
    args = json_to_args(cfg_file)
    model = RAFT(args)
...
def get_flow(path, cfg_file, model, image1, image2):
    with torch.no_grad():
      args = json_to_args(cfg_file)
      flow, info = calc_flow(args, model, image1.unsqueeze(0), image2.unsqueeze(0))
