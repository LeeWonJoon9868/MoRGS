...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, latent_args: QuantizeParams, model_args: ModelParams, frame_idx : int = 1, use_xyz_legacy: bool = False):
        ...
        self.xyz_before = None
        self.resi_before = None
        self._offset = None
        ...
        self.mask_motion_label = torch.empty(0)
        ...
        # self.viewspace_grad_diff_mask = None
        # self.dynamic_mask = None
        
        self.motion_label_compressed_threshold= 0.5

        # Learned per-Gaussian 3D offset attribute (frame >= 2).
        # Distinct from `_resi` which is the *computed* residual (xyz - xyz_before).
        self.offset_3d = None
    ...
    def strip_groups_by_name(self,opt_dict, patterns=(r'\bgru\b', r'\bmotion_gru\b', r'\bgru_')):
        """
        opt_dict에서 name이 patterns에 매칭되는 param_group들을 제거하고,
        해당 그룹의 params(old ids)에 해당하는 state도 함께 제거.
        """
        od = copy.deepcopy(opt_dict)
        pg = od.get("param_groups", [])
        st = od.get("state", {})

        # 남길/제거할 그룹 인덱스 구분
        def match(name):
            return any(re.search(p, str(name)) for p in patterns)

        keep_idx = [i for i,g in enumerate(pg) if not match(g.get("name", ""))]
        drop_idx = [i for i in range(len(pg)) if i not in keep_idx]

        # 남기는 그룹/제거되는 그룹의 old param ids
        kept_old_ids = set(chain.from_iterable(pg[i]["params"] for i in keep_idx))
        #drop_old_ids = set(chain.from_iterable(pg[i]["params"] for i in drop_idx))

        # param_groups 갱신
        od["param_groups"] = [pg[i] for i in keep_idx]

        # state도 kept ids만 남기기
        # (PyTorch는 saved old id → current param을 '그룹/순서'로 매핑합니다.
        # 그래서 남긴 그룹들의 ids만 state에 있어야 매핑이 깔끔해요.)
        if isinstance(st, dict):
            # keys가 int 또는 str(int)일 수 있음
            def _to_int(k):
                try:
                    return int(k)
                except Exception:
                    return k  # 예상 외 포맷은 그대로 두되 보통 int/str(int)
            od["state"] = {k: v for k, v in st.items() if _to_int(k) in kept_old_ids}
    ...
    def restore(self, model_args, training_args):
        ...
        self.denom = denom
        opt_no_gru = self.strip_groups_by_name(opt_dict, patterns=(r'\bgru\b', r'\bmotion_gru\b', r'\bgru_'))
        self.optimizer.load_state_dict(opt_no_gru)
        ...
        self.mask_motion_label.data = mask_atts["motion_label"]
    ...
    def get_decoded_atts(self):
        return OrderedDict({"xyz"     : self._xyz,
                            ...
                            "op"      : self._opacity,
                            "flow"    : self._flow,
                            "motion_label" : self._motion_label})
    ...
    def get_masks(self):
        return OrderedDict({"xyz"     : self.mask_xyz, 
                            ...
                            "flow"    : self.mask_flow,
                            "motion_label" : self.mask_motion_label})
    ...
    def get_frz(self):
        return OrderedDict({"xyz"     : self.frz_xyz, 
                            ...
                            "op"      : self.frz_opacity,
                            "flow"    : self.frz_flow,
                            "motion_label" : self.frz_motion_label})
    @property
    def mask_all(self):
        mask_all = torch.cat((self.mask_xyz, self.mask_features_dc, self.mask_features_rest,
                              self.mask_scaling, self.mask_rotation, self.mask_opacity, self.mask_flow, self.mask_motion_label),dim=1)
        return mask_all
    ...
    @property
    def _resi(self):
        # Actual per-Gaussian frame-to-frame residual displacement, masked by motion confidence.
        # Returns None on the first frame (no previous xyz to diff against).
        xyz = self.latent_decoders["xyz"](self._latents["xyz"])
        if self.prev_atts["xyz"] is not None:
            resi = (xyz - self.xyz_before[self.mapping]) * self.get_motion_label
        else:
            resi = None
        return resi
        ...
        #     resi = self.gate_atts((xyz-self.xyz_before[self.mapping])) * self.get_motion_label - (self._3d_offset[self.mapping] * self.get_motion_label)
        # else:
        #     resi=None
        # return resi
    ...
    def _xyz_fixed(self):
        ...
        if self.offset_3d is not None:
            xyz = xyz + (self.offset_3d) 

        if self.prev_atts["xyz"] is not None and self.xyz_before is not None: 
            resi = (xyz-self.xyz_before[self.mapping]) * self.get_motion_label
            xyz = resi + self.xyz_before[self.mapping]  
        else:
            return xyz
        return xyz
        ...
        #     try:
        #         xyz = self.gate_atts((xyz-self.xyz_before[self.mapping])) * self.get_motion_label + self.xyz_before[self.mapping] - (self._3d_offset[self.mapping] * self.get_motion_label)
        #     except:
    ...
    @property
    def _motion_label(self):
        """Get opacity values with optional gating."""
        motion_label = self.latent_decoders["motion_label"](self._latents["motion_label"])
        if self.prev_atts["motion_label"] is not None and self.gate_atts is not None and self.gate_params["motion_label"]:
            motion_label = self.gate_atts(motion_label-self.prev_atts["motion_label"])+self.prev_atts["motion_label"]
        return motion_label
    ...
    @property
    def get_offset_3d(self):
        return self.offset_3d
    ...
    def get_motion_label(self):
        return self.opacity_activation(self._motion_label)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, ignore_colors: bool = False):
        ...
        motion_labels = torch.zeros((fused_point_cloud.shape[0], 1), device="cuda")
        ...
        if self.latent_args.motion_label_invert_type == "autoenc" and not isinstance(self.latent_decoders["motion_label"], DecoderIdentity):
            init, decoder_state_dict = init_latents(self.latent_args, motion_labels,"motion_label", lambda_distortion=0.0)
            self.latent_decoders["motion_label"].load_state_dict(decoder_state_dict)
        else:
            init = self.latent_decoders["motion_label"].invert(motion_labels)
        self._latents["motion_label"] = nn.Parameter(init.requires_grad_(True))
        ...
        self.mask_motion_label.data = torch.ones_like(self._motion_label).bool()
    ...
    def create_from_depth_immersive(self, cameras, spatial_lr_scale, downsample_scale=2, alpha_thresh=0.1, renderFunc=None):
        ...
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        motion_labels = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        ...
        new_rotation = self.latent_decoders["rot"].invert(rots)
        new_opacity = self.latent_decoders["op"].invert(opacities)
        new_motion_label = self.latent_decoders["motion_label"].invert(motion_labels)
        new_flow = torch.zeros_like(fused_point_cloud)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_flow, new_motion_label)
        ...
        with torch.no_grad():
            ...
            for param_name in self.param_names:
                ...
                if isinstance(decoder, DecoderIdentity) \
                    ...
                    # if param_name == "xyz":
                    #     mask = (self.get_motion_label > self.motion_label_compressed_threshold)
                    #     mask_frac = (mask!=0.0).sum().item()/mask.numel()
    ...
    def training_setup(self, training_args):
        ...
        self.orig_lr = OrderedDict({"xyz":training_args.position_lr_init, 
                                    ...
                                    "f_rest":training_args.features_rest_lr, 
                                     ...
                                     #"_3d_offset": training_args.offset_lr_init,
                                     "motion_label": training_args.opacity_lr}) #Fixed
        lr = {
                ...
                #'_3d_offset': training_args.offset_lr_init * self.spatial_lr_scale, #* self.lr_scaling["offset"],
                'motion_label': training_args.opacity_lr*self.lr_scaling["op"], #Fixed
            }
        l = []
        for i,param in enumerate(self.param_names):
        ...
        #     l += [{"params": list(self._offset), "lr": training_args.offset_lr_init, "name": "_offset"}]
            
        if hasattr(self, "offset_3d") and self.offset_3d is not None:
            l += [{"params": self.offset_3d, "lr": training_args.offset_lr_init, "name": "offset_3d"}]
    ...
    def update_masks(self, args, dynamic_mask):
        frz_args = {"xyz":args.frz_xyz, "f_dc":args.frz_f_dc, "f_rest":args.frz_f_rest, 
                    "op":args.frz_op, "rot":args.frz_rot, "sc":args.frz_sc, "flow":args.frz_flow, "motion_label": args.frz_motion_label}
    ...
    def freeze_atts(self, args):
        ...
        self.frz_motion_label = args.frz_motion_label
    ...
    def unfreeze_atts(self, args):
        # 이전 상태
        prev_frz = {
            "xyz": self.frz_xyz, "f_dc": self.frz_features_dc, "f_rest": self.frz_features_rest,
            "sc": self.frz_scaling, "rot": self.frz_rotation, "op": self.frz_opacity,
            "flow": self.frz_flow, "motion_label": self.frz_motion_label,
        }
        # 이번에 전부 none으로 풀 계획이라면 이렇게 하셔도 되고,
        # 실제 args에서 읽어오려면 next_frz를 args 기반으로 채우세요.
        next_frz = {k: "none" for k in prev_frz.keys()}

        atts = self.get_atts  # <--호출

        for att_name, prev_mode in prev_frz.items():
            new_mode = next_frz[att_name]
            if prev_mode in ("st", "all") and new_mode == "none":
                if att_name in atts:
                    atts[att_name].requires_grad_(True)

                if hasattr(self, "latent_decoders") and att_name in self.latent_decoders:
                    dec = self.latent_decoders[att_name]
                    if dec is not None:
                        # LatentDecoder/LatentDecoderRes에 unfreeze_partial() 구현해둔 경우
                        if hasattr(dec, "unfreeze_partial") and callable(dec.unfreeze_partial):
                            try:
                                dec.unfreeze_partial()
                            except Exception:
                                pass
        ...
        # 내부 상태 갱신
        self.frz_xyz           = next_frz["xyz"]
        self.frz_features_dc   = next_frz["f_dc"]
        self.frz_features_rest = next_frz["f_rest"]
        self.frz_scaling       = next_frz["sc"]
        self.frz_rotation      = next_frz["rot"]
        self.frz_opacity       = next_frz["op"]
        self.frz_flow          = next_frz["flow"]
        self.frz_motion_label  = next_frz["motion_label"]
    ...
    def update_residuals(self):
        ...
        #self.resi_before = self.get_resi.clone()
        
        if self.offset_3d is None:
            self.offset_3d = nn.Parameter(torch.zeros(self.xyz_before.shape[0], 3, device="cuda"), requires_grad=True)
    ...
    def update_learning_rate(self, iteration, latent_args: QuantizeParams):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            # elif param_group["name"] == "_offset":
            #     lr = self.offset_scheduler_args(iteration)
            #     param_group['lr'] = lr
                
            # elif param_group["name"] == "_3d_offset":
            #     lr = self.offset_scheduler_args(iteration)
            #     param_group['lr'] = lr
    ...
    def save_ply(self, path, mask):
        ...
        if mask is not None:
            ...
            motion_labels = self._motion_label[mask].detach().cpu().numpy()
        else:
            ...
            motion_labels = self._motion_label.detach().cpu().numpy()
        ...
        dtype_full.extend([('motion_label', 'f4')])
        dtype_full.extend([(attribute, 'f4') for attribute in 
        ...
        elements['motion_label'] = motion_labels[:, 0]  # Fix the shape here
    ...
    def load_ply(self, path, verbose=False):
        ...
        motion_labels = np.asarray(plydata.elements[0]["motion_label"])[..., np.newaxis]
        ...
        self._latents["motion_label"] = nn.Parameter(torch.tensor(motion_labels, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def load_ply2(self, path, verbose=False):
        plydata = PlyData.read(path)
        if verbose:
            print(f"GaussianModel::load_ply(): loaded gaussian from ply file at: {path}")
    ...
    def _prune_optimizer(self, mask):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if "ldec" in group["name"]: #or "_offset" in group["name"]:
                continue
            stored_state = self.optimizer.state.get(group['params'][0], None)
    ...
    def prune_points(self, mask):
        ...
        if self.offset_3d is not None:
            self.offset_3d = self.offset_3d[valid_points_mask]
        ...
        # self.gaussian_ids = self.gaussian_ids[valid_points_mask]
                    
    def cat_tensors_to_optimizer(self, tensors_dict):
        optimizable_tensors = {}
        masks = self.get_masks
        for group in self.optimizer.param_groups:
            if "ldec" in group["name"]: #or "_offset" in group["name"]: #or "_3d_offset" in group["name"]:
                continue
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_flow, new_motion_label, new_offset_3d,
        ...
        "rot" : new_rotation,
        "flow":  new_flow,
        "motion_label": new_motion_label,
        "offset_3d": new_offset_3d}
        ...
        if self.offset_3d is not None:
            self.offset_3d = optimizable_tensors["offset_3d"]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, is_dynamic=False):
        ...
        new_motion_label = self._latents["motion_label"][selected_pts_mask].repeat(N,1)
        
        if self.offset_3d is not None:
            new_offset_3d = self.offset_3d[selected_pts_mask].repeat(N,1)
        else:
            new_offset_3d = None
        
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_flow, new_motion_label, new_offset_3d,
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, is_dynamic=False):
        ...
        new_motion_labels = self._latents["motion_label"][selected_pts_mask]
        
        if self.offset_3d is not None:
            new_offset_3d = self.offset_3d[selected_pts_mask]
        else:
            new_offset_3d = None
        
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_flow, new_motion_labels, new_offset_3d,
    ...
    def add_densification_stats3(self, viewspace_point_tensor, update_filter, mask):
        self.xyz_gradient_accum[mask][update_filter] += torch.norm(viewspace_point_tensor.grad[mask][update_filter,:2], dim=-1, keepdim=True)
        self.denom[mask][update_filter] += 1

    def add_densification_stats2(self, viewspace_point_tensor, update_filter, grad_override=None):
        grad = grad_override if grad_override is not None else viewspace_point_tensor.grad
    ...
    def copy(self):
        ...
        new_model.mask_motion_label.data = self.mask_motion_label.data.clone()
        ...
        new_model.frz_motion_label = self.frz_motion_label
