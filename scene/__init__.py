...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, train_image_data=None, test_image_data=None,
                 load_iteration=None, shuffle=True, resolution_scales=[1.0], N_video_views=300, verbose=False, resolution=1.0, load_init_pcd=None):
        """b
        ...
        self.load_init_pcd = load_init_pcd
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["Dynerf"](args.source_path, args.test_indices, resolution, N_video_views)
            dataset_type="dynerf"
            ...
            scene_info = sceneLoadTypeCallbacks["Immersive"](args.source_path, args.test_indices, resolution,
                                                             args.world_scale)
        ...
        if self.loaded_iter:
            self.gaussians.load_ply(os.path.join(self.model_path,
                                                ...
                                                "point_cloud.ply"))
            # self.gaussians.load_ply(os.path.join(self.loaded_iter,
            #                                     "point_cloud",
            #                                     "iteration_10000",
            #                                     "point_cloud.ply"))            
            
        elif self.load_init_pcd:
            self.gaussians.load_ply2(self.load_init_pcd)
    ...
    def updateCameraImages(self, args, train_image_data, test_image_data, frame_idx, resolution_scales=1.0):
        for resolution_scale in resolution_scales:
            for idx,camera in enumerate(self.train_cameras[resolution_scale]):
                ...
                mask = train_image_data['mask'][idx]
                updateCam(args, image, train_image_data['path'][idx], mask, frame_idx, camera, resolution_scale)
            for idx,camera in enumerate(self.test_cameras[resolution_scale]):
                image = test_image_data['image'][idx]
                mask = None
                updateCam(args, image, test_image_data['path'][idx], mask, frame_idx, camera, resolution_scale)
