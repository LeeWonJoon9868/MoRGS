...
def save_mask(
    ...
    # tensor: (1,H,W) or (H,W), 값이 [0,1]
    if tensor.ndim == 3:  # (1,H,W)
        tensor = tensor.squeeze(0)

    ndarr = torch.round(tensor * 255).to("cpu", torch.uint8).numpy()  # (H,W)
    im = Image.fromarray(ndarr, mode="L")  # "L" = 8bit grayscale
    im.save(fp)
...
def save_motion(
    ...
    ndarr = torch.round(tensor).permute(1,2,0).to("cpu", torch.uint8).numpy()
    im = Image.fromarray(ndarr)
    im.save(fp)

def write_depth(path, depth, grayscale=True, bits=1):
...
def flow_tensor_to_image(flow):
    ...
    #flow = torch.from_numpy(flow).permute(2,0,1)  # [3, H, W]
    return flow
