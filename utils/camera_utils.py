...
def updateCam(args, image, image_path, mask, frame_idx: int, cam: SequentialCamera, resolution_scale: float):
    ...
    if isinstance(image, torch.Tensor):
        ...
        # if args.resolution in [1, 2, 4, 8]:
        #     new_w = round(orig_w / (resolution_scale * args.resolution))
        #     new_h = round(orig_h / (resolution_scale * args.resolution))
        # else:
        #     if args.resolution == -1:
        #         if orig_w > 1600:
        #             if not WARNED:
        ...
        #     image.unsqueeze(0), size=(new_h, new_w), mode="bicubic", align_corners=False
        # ).squeeze(0)        
...
def loadSequentialCam(args, id, cam_info:SequentialCameraInfo, resolution_scale, image=None, image_path=None, mask=None, frame_idx=None):
