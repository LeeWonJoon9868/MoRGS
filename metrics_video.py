...
def evaluate(model_paths, wandb_project):
    ...
    for scene_dir in model_paths:
        ...
        for cam_name in os.listdir(gt_dir):
            ...
            #renders, gts, image_names = renders[:1], gts[:1], image_names[:1]
        ...
        training_metrics_path = Path(scene_dir) / "training_metrics.json"
        if training_metrics_path.exists():
            with open(training_metrics_path, "r") as f:
                train_metrics = json.load(f)

            # 기존
            sizes = [item.get("Size (MB)") for item in train_metrics if "Size (MB)" in item]
            frame_times = [item.get("Frame time") for item in train_metrics if "Frame time" in item]
            frame_time_ios = [item.get("Frame time IO") for item in train_metrics if "Frame time IO" in item]



            if sizes:        avg_view_dict["Avg Size (MB)"]      = sum(sizes) / len(sizes)
            if frame_times:  avg_view_dict["Avg Frame time"]     = sum(frame_times) / len(frame_times)
            if frame_time_ios: avg_view_dict["Avg Frame time IO"] = sum(frame_time_ios) / len(frame_time_ios)
