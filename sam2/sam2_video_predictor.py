...
def preprocess_images_like_loader(
    images,                    # list[np.ndarray(H,W,3) or torch.Tensor], 또는 torch.Tensor (T,H,W,3)/(T,3,H,W)
    image_size: int,
    compute_device: torch.device,
    offload_video_to_cpu: bool,
):
    """
    load_video_frames_from_jpg_images 와 동일한 결과:
    - (T, 3, image_size, image_size) float32
    - (x - mean) / std 정규화
    - offload_video_to_cpu=False면 GPU로 이동
    반환: images_norm, video_height, video_width
    """
    assert image_size % 16 == 0, "image_size는 16의 배수 권장(힐라 백본 윈도우 맞춤)."

    def _to_chw_and_resize(x):
        if isinstance(x, np.ndarray):
            x = torch.from_numpy(x)
        # HWC → CHW
        if x.ndim == 3 and x.shape[-1] == 3:
            x = x.permute(2, 0, 1)
        assert x.ndim == 3 and x.shape[0] == 3, f"Expect CHW/HWC 3ch, got {tuple(x.shape)}"
        # uint8 → [0,1] float32
        if x.dtype == torch.uint8:
            x = x.float().div_(255.0)
        else:
            x = x.float()
            if x.max() > 1.5:  # 0~255였던 경우
                x = x / 255.0
        # (3,H,W) → 리사이즈
    ...
    if isinstance(images, torch.Tensor):
        if images.ndim == 4 and images.shape[-1] == 3:     # (T,H,W,3)
            images = images.permute(0, 3, 1, 2)            # (T,3,H,W)
        assert images.ndim == 4 and images.shape[1] == 3
        frames = [_to_chw_and_resize(images[t]) for t in range(images.shape[0])]
        # 원본 해상도(첫 프레임 기준)
        video_height, video_width = int(images.shape[2]), int(images.shape[3])
    else:
        # list of HWC/CHW
        frames = [_to_chw_and_resize(f) for f in images]
        # 원본 해상도(첫 프레임 기준; HWC 가정)
        f0 = images[0]
        if isinstance(f0, np.ndarray):
            video_height, video_width = int(f0.shape[0]), int(f0.shape[1])
        else:
            # torch 텐서인 경우
            if f0.ndim == 3 and f0.shape[-1] == 3:      # HWC
                video_height, video_width = int(f0.shape[0]), int(f0.shape[1])
            else:                                       # CHW
                video_height, video_width = int(f0.shape[1]), int(f0.shape[2])

    images_chw = torch.stack(frames, dim=0)  # (T,3,S,S)


    return images_chw, video_height, video_width
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state2(
        ...
        compute_device = self.device  # device of the model
        # images, video_height, video_width = load_video_frames(
        #     video_path="/SSD1/wonjoon/sam2/notebooks/videos/bedroom",
        #     image_size=self.image_size,
        #     offload_video_to_cpu=offload_video_to_cpu,
        #     async_loading_frames=async_loading_frames,
        #     compute_device=compute_device,
        # )
        #video_height, video_width = images[0].shape[1:]
        images, video_height, video_width = preprocess_images_like_loader(
            images=images,
            image_size=self.image_size,            # ← loader와 동일하게 SxS로 맞춤
            compute_device=compute_device,
            offload_video_to_cpu=offload_video_to_cpu,
        )
        inference_state = {}
    ...
    def init_state3(
        ...
        if prev_inference_state is None:
            ...
            inference_state = prev_inference_state
            # 기존 state에서 이미지 관련 부분만 교            inference_state["images"] = images
            inference_state["num_frames"] = len(images)
            inference_state["video_height"] = video_height
            inference_state["video_width"] = video_width
