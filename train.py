...
from scene.cameras import camName_from_Path
from scene.utils import get_flow_model, get_flow, get_sam2_model, get_sam2_mask
...
from utils.general_utils import safe_state, adaptive_multiotsu_variance
from utils.image_utils import psnr, save_image, save_motion, write_depth, resize_dims, flow_tensor_to_image
...
def training(dataset: ModelParams, opt: OptimizationParams, pipe: PipelineParams, qp:QuantizeParams, testing_iterations: list, 
             saving_iterations: list, checkpoint_iterations, checkpoint: str, load_init: bool, debug_from, args):
    ...
    train_image_dataset = MultiViewVideoDataset(dataset.source_path, split='train', test_indices=dataset.test_indices,
                                                max_frames=dataset.max_frames, start_idx=dataset.start_idx, img_format=dataset.img_fmt, resolution=dataset.resolution)
    test_image_dataset = MultiViewVideoDataset(dataset.source_path, split='test', test_indices=dataset.test_indices, 
                                               ...
                                               img_format=dataset.img_fmt, resolution=dataset.resolution)
    ...
    train_images, train_paths, train_mask_images = train_data
    if test_image_dataset.n_cams > 0:
        test_data = next(test_loader)
        test_images, test_paths, test_mask_images = test_data
        test_image_data = {'image':test_images.cuda(),'path':test_paths,'frame_idx':0, 'mask': test_mask_images.cuda()}
    else:
        print('No test cameras found, disabling testing.')
        test_images, test_paths = None, None
        test_image_data = {'image':None,'path':None,'frame_idx':0, 'mask': test_mask_images.cuda()}

    train_image_data = {'image':train_images.cuda(),'path':train_paths,'frame_idx':0, 'mask': train_mask_images.cuda()}
    ...
    if load_init:
        checkpoint = os.path.join(dataset.source_path, "init/frames/0001/chkpnt.pth" )
        load_init_pcd = os.path.join(dataset.source_path, "init/frames/0001/point_cloud.ply" )
    ...
    flow_model = get_flow_model(args.flow_model_ckpt, args.flow_config)
    sam2_predictor = get_sam2_model(args.sam2_checkpoint, args.sam2_model_cfg)
    ...
    for frame_idx in range(start_frame_idx, n_frames+1):
        ...
        # Select KeyFrames For Optical Flow
        def peek_frame_batch(dataset, views, f):
            idxs = [c * dataset.n_frames + f for c in views]
            imgs, paths, _ = zip(*(dataset[i] for i in idxs))  # [(C,H,W), str] * n_cams
            return torch.stack(imgs).cuda(), paths

        # if "flame_steak" in dataset.source_path:
        #     dataset.flow_views = [4, 10, 11, 15]
        # if "sear_steak" in dataset.source_path:
        #     dataset.flow_views = [5, 6, 15, 16]

        if (frame_idx % dataset.keyframe_offset_op == 0) or (frame_idx==1) or (frame_idx==2):
            flow_offset = dataset.flow_offset 
            if frame_idx==1:
                flow_offset=3
                peek_f = min((frame_idx-1) + flow_offset, max_frames-1)
            else:
                peek_f = min((frame_idx-1) - flow_offset, max_frames-1)
            peek_gt_images, peek_gt_paths = peek_frame_batch(train_image_dataset, dataset.flow_views, peek_f) 
        ...
        try: 
            ...
            next_train_images, next_train_paths, next_train_masks = next_train_data[0].cuda(), next_train_data[1], next_train_data[2]
        ...
        if (frame_idx % dataset.keyframe_offset_op == 0) or (frame_idx==1) or (frame_idx==2):
            ...
            for idx, camera in enumerate(train_cameras):
                if camera.uid not in dataset.flow_views:
                    continue  
                else:
                    cur_gt_image = cur_frame_views[idx]
                    peek_gt_image = peek_gt_images[peek_idx]
                    peek_idx += 1
                    with torch.no_grad():
                        if frame_idx==1:
                            flow, info, heatmap = get_flow(args.flow_model_ckpt, args.flow_config, flow_model, cur_gt_image, peek_gt_image)
                        else:
                            flow, info, heatmap = get_flow(args.flow_model_ckpt, args.flow_config, flow_model, peek_gt_image, cur_gt_image)
                        train_cameras[idx].optical_flow = flow[0]
                        train_cameras[idx].heatmap = heatmap[0]
                        
                        flow_mag = torch.sqrt(flow[0][0] ** 2 + flow[0][1] ** 2)
                        train_cameras[idx].optical_flow_mask =  flow_mag > dataset.flow_thresh1 #adaptive_multiotsu_variance(flow_mag) #dataset.flow_thresh1
                        optical_flow_masks.append(train_cameras[idx].optical_flow_mask)
                        cur_flow_gt_images.append(cur_gt_image)
            

            torch.cuda.synchronize()
            t_flow1 = time.time()
            flow_compute_time = t_flow1 - t_flow0
            torch.cuda.synchronize()
            t_sam0 = time.time()
        
        if (frame_idx % dataset.keyframe_offset_sam == 0) or (frame_idx==1) or (frame_idx==2):
            optical_flow_sam2_masks = get_sam2_mask(sam2_predictor, cur_flow_gt_images, optical_flow_masks)
            
            torch.cuda.synchronize()
            t_sam1 = time.time()
            sam2_time = t_sam1 - t_sam0

            sam_idx = 0
            for idx, camera in enumerate(train_cameras):
                if camera.uid  in dataset.flow_views: # and camera.uid != 1
                    camera.optical_flow_sam2_mask = optical_flow_sam2_masks[sam_idx]
                    camera.optical_flow_sam2_mask_final = optical_flow_sam2_masks[sam_idx] + optical_flow_masks[sam_idx] 
                    sam_idx += 1
                else:
                    continue
        ...
        if frame_idx > 1: 
            with torch.no_grad():
                ...
                train_images, train_paths, train_masks = cur_train_images, cur_train_paths, cur_train_masks
                if dataset.timed:
                    torch.cuda.synchronize()
                frame_time += time.time() - frame_start
                if test_image_dataset.n_cams > 0:
                    test_data = next(test_loader)
                    test_images, test_paths, test_masks = test_data[0].cuda(), test_data[1], test_data[2].cuda()
                else:
                ...
                if dataset.timed:
                    torch.cuda.synchronize()
                frame_start = time.time()
                train_image_data = {'image':train_images,'path':train_paths, 'mask':train_masks}
                test_image_data = {'image':test_images,'path':test_paths, 'mask':test_masks}
                ...
                if dataset.adaptive_render and dataset.adaptive_update_period>0.0: # True (함)
        ...
        for iteration in range(first_iter, opt.iterations + 1):        
            ...
            if frame_idx == 1:
                ...
                if (viewpoint_cam.uid in dataset.flow_views) and  ((frame_idx % dataset.keyframe_offset_op == 0) or (frame_idx==2)): 
                    render_flow=True
                else:
                    render_flow=False
                    
            if frame_idx == 1:
                if (viewpoint_cam.uid in dataset.flow_views) and (iteration>opt.motion_label_from_iter and iteration<=opt.motion_label_until_iter): 
                    render_motion_label = True
                else:
                    render_motion_label = False
            else:
                if (viewpoint_cam.uid in dataset.flow_views) and  ((frame_idx % dataset.keyframe_offset_sam == 0) or (frame_idx==2)): 
                    render_motion_label=True
                else:
                    render_motion_label=False
            ...
            render_pkg = render_mask(viewpoint_cam, gaussians, pipe, bg, image_shape=gt_image.shape, 
                                     ...
                                     update_mask=None,
                                     render_motion_label=render_motion_label)
            ...
            if iteration>opt.color_from_iter:
                if not torch.all(viewpoint_cam.gt_alpha_mask==1):
                    Ll1 = l1_loss(image*viewpoint_cam.gt_alpha_mask, gt_image*viewpoint_cam.gt_alpha_mask)
                    Lssim = ssim(image*viewpoint_cam.gt_alpha_mask, gt_image*viewpoint_cam.gt_alpha_mask)    
                ...
                recon_loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - Lssim)
                loss += recon_loss
            
            if render_flow==True: 
                flow2d = render_pkg["flow"] #+ offset
                optical_flow = viewpoint_cam.optical_flow.clone() 
                optical_flow_mask = viewpoint_cam.optical_flow_sam2_mask_final

                alpha_sum  = render_pkg["alpha"]          
                eps = 1e-6
                flow2d = flow2d / (alpha_sum + eps)
                flow_loss =  (((flow2d) - optical_flow).pow(2).sum(dim=0) + eps**2).sqrt().mean()
                loss += 0.01 * flow_loss 

            # 3D Motion Residual Field Regularization
            if frame_idx > 1:
                loss +=  torch.abs(gaussians.get_offset_3d).mean() 
                
            if (render_motion_label == True) :
                pred_motion_label = render_pkg["rendered_motion"].squeeze(0)
                gt_motion_label = (viewpoint_cam.optical_flow_sam2_mask_final).float()
                loss_motion = l1_loss(pred_motion_label, gt_motion_label)
                loss += 0.1 * loss_motion
                
            if load_init and frame_idx ==1:
                pass
            else:
                loss.backward()
            ...
            with torch.no_grad():
                ...
                if not load_init and frame_idx == 1:
                ...
                if frame_idx > 1 :
                    if iteration <= (np.ceil(opt.densify_until_epoch*opt.iterations)) and iteration>(opt.calc_dense_stats*n_cams):
                        ...
                        if iteration % (opt.densification_interval*n_cams) == 0:
                            size_threshold = opt.size_threshold if iteration > (opt.opacity_reset_interval*n_cams) else None
        ...
        if frame_idx != n_frames:
            ...
            cur_train_masks = next_train_masks
        ...
        if test_image_dataset.n_cams > 0 :
            frame_metrics = {
                ...
                "FPS (Test)":  round(float(metrics['test'].get('fps', 0.0)), 3),  # <- 추가
                "FPS (Val)":   round(float(metrics['val'].get('fps', 0.0)), 3),   # <- 추가
                "Frame time": round(frame_time,2),
                "Frame time IO": round(frame_time_io,2),
                "Training time elapsed": round(net_elapsed_time,2),

                "OPTICAL FLOW TIME": round(flow_compute_time,2),
                "SAM2 TIME": round(sam2_time,2),
            }
        else:
...
def training_report(tb_writer, wandb_enabled, model_args, frame_idx, iteration, Ll1, loss, l1_loss, size, 
    ...
    if is_test:
        ...
        validation_configs = ({'name': 'test', 'cameras' : scene.getTestCameras()},
                              {'name': 'val', 'cameras': scene.getTrainCameras()[0:1]})
        ...
        for config in validation_configs:
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                if model_args.log_images: 
                    ...
                    os.makedirs(os.path.join(model_args.model_path,config['name'],"offset"),exist_ok=True)
                    os.makedirs(os.path.join(model_args.model_path,config['name'],"optical_flow_sam2_mask_final"),exist_ok=True)
                    os.makedirs(os.path.join(model_args.model_path,config['name'],"motion_label"),exist_ok=True)
                    os.makedirs(os.path.join(model_args.model_path,config['name'],"motions"),exist_ok=True)

                for idx, viewpoint in enumerate(config['cameras']):
                    if frame_idx > 1 :
                        render_pkg = renderFunc(viewpoint, scene.gaussians, *renderArgs, motion=True, render_depth=True, render_motion_label=True, render_flow=True)
                        # offset_image: rasterised projection of pc.offset_3d (learned per-Gaussian 3D offset)
                        # motion_image: rasterised projection of pc._resi (actual frame-to-frame residual flow)
                        offset_image = render_pkg["flow_pred"]
                        motion_image = render_pkg["flow"]

                    else:
                        render_pkg = renderFunc(viewpoint, scene.gaussians, *renderArgs, render_depth=True, render_motion_label=True)
                        offset_image = None
                        motion_image = None
                    ...
                    if viewpoint.gt_depth is not None:
                        gt_depth=viewpoint.gt_depth.cpu().numpy()
                        
                    render_depth=render_pkg["depth"].cpu().numpy()
                    motion_label = render_pkg["rendered_motion"].squeeze(0).cpu().numpy()
                    ...
                    if model_args.log_images:
                        ...
                        os.makedirs(os.path.join(model_args.model_path,config['name'],"offset",
                                                camName_from_Path(viewpoint.image_path)),exist_ok=True)
                        os.makedirs(os.path.join(model_args.model_path,config['name'],"motion_label",
                                                camName_from_Path(viewpoint.image_path)),exist_ok=True)
                        os.makedirs(os.path.join(model_args.model_path,config['name'],"motions",
                                                camName_from_Path(viewpoint.image_path)),exist_ok=True)
                        ...
                        save_image(gt_image,os.path.join(model_args.model_path,config['name'],"gt", 
                        ...
                        write_depth(os.path.join(model_args.model_path,config['name'],"motion_label", 
                                                       camName_from_Path(viewpoint.image_path),str(model_args.start_idx+frame_idx).zfill(4)+".png"), motion_label)
                        if motion_image is not None:
                            motion_image = flow_tensor_to_image(motion_image)
                            save_motion(motion_image, os.path.join(model_args.model_path, config['name'], "motions",
                                                                   camName_from_Path(viewpoint.image_path), str(model_args.start_idx+frame_idx).zfill(4)+".png"))

                        if offset_image is not None:
                            offset_image = flow_tensor_to_image(offset_image)
                            save_motion(offset_image, os.path.join(model_args.model_path, config['name'], "offset",
                                                                   camName_from_Path(viewpoint.image_path), str(model_args.start_idx+frame_idx).zfill(4)+".png"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_init", action='store_true', default=False)
    ...
    training(lp_args, op, pp_args, qp_args, args.test_iterations, args.save_iterations, 
             args.checkpoint_iterations, args.start_checkpoint, args.load_init, args.debug_from, args)
