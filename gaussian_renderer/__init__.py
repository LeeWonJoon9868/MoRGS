...
def render_mask(viewpoint_camera: SequentialCamera, pc : GaussianModel, pipe, bg_color : torch.Tensor, 
                ...
                retain_grad=False, update_mask=None, gaussian_mask=None, render_motion_label = False, motion=False):
    """
    Render the scene. 
    ...
    #_xyz, _resi, _scale, _rotation = pc.get_geometry

    means3D = pc.get_xyz #_xyz #pc.get_xyz
    
    if render_flow is False or pc.frame_idx==1:
        flow3D  = pc.get_flow
    else:
        flow3D = pc.get_resi #_resi #pc.get_resi #pc.get_resi_prior #pc.get_resi

    means2D = screenspace_points
    opacity = pc.get_opacity
    
    # if render_motion_label:
    #     motion_label = pc.get_motion_label
    # else:
    #     motion_label = pc.get_motion_label.detach()
    motion_label = pc.get_motion_label
    ...
    if pipe.compute_cov3D_python:
        ...
        scales = pc.get_scaling #_scale #pc.get_scaling
        rotations = pc.get_rotation #_rotation #pc.get_rotation
    ...
    if gaussian_mask is not None:        
        rendered_image, rendered_motion, flow2D, infl, count_infl, depth, alpha, radii = rasterizer(
            ...
            motion_label = motion_label[gaussian_mask], #if motion_label else motion_label,
    ...
    if raster_settings.render_motion_label:
        render_pkg["rendered_motion"] = rendered_motion
 
    if motion:
        with torch.autocast(device_type=pc.get_xyz.device.type, dtype=torch.float32):
            ...
            resi_xyz = pc.get_offset_3d #pc.get_3d_offset #pc.get_resi_prior #_resi #pc.get_resi #pc.get_resi #pc.get_resi
            flow_cam = resi_xyz@viewpoint_camera.world_view_transform[:3,:3]
            fx, fy = fov2focal(viewpoint_camera.FoVx,image_shape[2]), fov2focal(viewpoint_camera.FoVy, image_shape[1])
            ...
            shs = None
            cov3D_precomp = None
            color_mask = (torch.ones_like(means3D, dtype=means3D.dtype, requires_grad=True, device="cuda") + 0).bool()
            ...
            if pc.frame_idx>1:
                ...
                # assert means2D.shape[0] == shs.shape[0]
                # assert opacity.shape[0] == shs.shape[0]
                assert means2D.shape[0] == colors_precomp.shape[0]
                assert opacity.shape[0] == colors_precomp.shape[0]
                assert opacity.shape[0] == rotations.shape[0]
                assert update_mask.shape[0] == rotations.shape[0]
                if pixel_mask is not None:
                    assert pixel_mask.shape[-2] == image_shape[1] and pixel_mask.shape[-1] == image_shape[2]
            ...
            if gaussian_mask is not None:        
                rendered_image, _, flow2D, infl, count_infl, depth, alpha, radii = rasterizer(
                    means3D = means3D[gaussian_mask],
                    flow3D = flow3D[gaussian_mask],
                    means2D = means2D[gaussian_mask],
                    shs = shs[gaussian_mask],
            ...
            render_pkg.update({"flow_pred":rendered_image[:2], "flow_mask":alpha})
